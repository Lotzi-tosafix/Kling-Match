...
# ── SplitSaveButton ───────────────────────────────────────────────────────────
# פורמטים זמינים: (מזהה, תווית תצוגה, תיאור לתפריט)
_FORMATS = [
...
# ── SettingsDialog ────────────────────────────────────────────────────────────
# ערך גלובלי — מצב בהיר (ברירת מחדל: כהה)
_LIGHT_MODE: bool = False


class SettingsDialog(QDialog):
    """
    פופאפ הגדרות — פורמט שמירה ברירת מחדל + מצב בהיר/כהה.
    המתג משנה את המראה מיד (תצוגה מקדימה חיה).
    ביטול — מחזיר למצב הקודם.
    אישור — מאשר את השינוי.
    """

    # נשלח כשהמתג מוחלף (תצוגה מקדימה חיה, לפני אישור)
    ...
    def __init__(self, current_format: str = "mp3",
        ...
        self._original_light = _styles.is_light_mode()   # שמור למקרה של ביטול
    ...
    def _section_title(self, text: str) -> QLabel:
        ...
        lbl.setStyleSheet(
            ...
            f" border-bottom: 1px solid {_styles.COLOR_OUTLINE_BRIGHT}; padding-bottom: 4px;"
        )
        return lbl
    ...
    def _setup(self, current_format: str) -> None:
        ...
        ac_lay = QHBoxLayout(appearance_card)
        ...
        for fmt_id, fmt_label, fmt_desc in _FORMATS:
            ...
            fc_lay = QHBoxLayout(fmt_card)
            fc_lay.setContentsMargins(14, 8, 14, 8)
    ...
    def _on_toggle_preview(self, light: bool) -> None:
        """מופעל מיד כשהמתג מוחלף — תצוגה מקדימה חיה."""
        self.preview_mode_changed.emit(light)

    def _on_cancel(self) -> None:
        """ביטול — החזר את המצב המקורי אם שונה."""
